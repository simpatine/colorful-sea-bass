...
for ntissue in 0,10,20:
    data = pd.read_csv(f"data/out_{ntissue}.txt", delimiter=";",header=0)
...
# plt.semilogx(vals.index,vals.f1,":",marker="",label="F1")
    ...
    plt.xscale("log")
    plt.title(f'Accuracy and F1 scores with random subsampling on genes with ntissue in [{ntissue},{ntissue+10})')
    plt.xlabel("Subsampling ratio")
    plt.ylabel("Accuracy/F1 score (%)")
    plt.grid()
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"subsample_ntissue_{ntissue}.png")
plt.show()
