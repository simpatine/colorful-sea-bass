...
The baseline results refer to the predictor that guesses the most frequent outcome.}
\label{fig:res1}
\end{figure}
...
\label{fig:res2b}
\end{subfigure}
\caption{Plots of the accuracy and F1 scores for the different subsampling rates on the annotated genes.
    Each single plot is done using only the genes annotated with the indicated values.
    The baseline results refer to the predictor that guesses the most frequent outcome.}
\label{fig:res2}
\end{figure}
...
Overall the results do not show any unexpected meaningful trend. The most prominent pattern to be inferred from our data is a flat dependence between the variables, meaning that the scores are unaffected by the subsampling rate. This seems pretty clear outside the extreme ends of our sampled interval of subsampling rates, especially the end with lower rates.
