...
\end{minipage}
\begin{minipage}{0.49\textwidth}
    \centering\includegraphics[width=0.64\linewidth]{figures/VNN.jpg}
    \centering\includegraphics[width=0.64\linewidth]{figures/Black-Sea-Bass-303435837.jpg}
\end{minipage}
...
\frametitle{SNPs for predicting mortality}  
  ...
  \centering The \textbf{genome} might be useful to predict mortality.

  \vspace{0.3cm}

\begin{minipage}[c]{0.45\textwidth}
  \includegraphics[width=0.7\linewidth]{figures/SNP.png}
\end{minipage}
...
\begin{tikzpicture}
    ...
    \foreach \i in {1,...,\bars} {
      ...
      \foreach \j in {1,...,\numsegments} {
        \pgfmathsetmacro{\segmentheight}{0.05+rnd*(\barheight-0.25)} % Random height for the segment
        
        % Draw the segment with random color
        \fill[blue, rounded corners=1pt] (\xpos, \segmentheight) rectangle (\xpos+0.5, \segmentheight+0.2);
      }
      
      \draw[thick, black, rounded corners=5pt] (\xpos, 0) rectangle (\xpos+0.5, \barheight);
    }
  \end{tikzpicture}
\end{frame}
...
\section{Results}
\begin{frame}{Results: testing setup}

\textbf{Baseline scores:}
In every plot we compared our scores with some "baseline scores". These baseline scores are the score of a "trivial" classifier, that always reports the most common class.
